...
- **Vald Fas 1-kodbas:** Bygger på kod skriven av `https://github.com/Martin-Joensson/projekt-agila-metoder-webshop`
- **Nytt gemensamt GitHub-repo:** `https://github.com/Martin-Joensson/Grupp5-Webshop`
- **Eventuella städnings- eller refaktoreringsbehov i basen innan start:**
  - Flytta in Admin sidan i /admin
...
- **Given** _att jag befinner mig på produktkatalogen_
- **When** _jag skriver "jacka" i sökfältet_
- **Then** _uppdateras URL:en till `?search=jacka` och endast produkter med "jacka" i titeln eller beskrivningen visas._
- **And** _om inga varor matchar visas ett tydligt meddelande: "Inga produkter matchade din sökning"._

#### User Story 2: Se produkter, priser och rabatt i varukorgen

- **Som en** _kund som är färdigshoppad_
- **vill jag** _kunna se produkterna som jag har i varukorgen samt vad dom kostar både totalt och enskilt inklusive rabatter_
- **så att** _vet vad jag ska betala i nästa steg._

**Acceptanskriterier:**

- **Given** _att jag befinner mig på kundkorgens sida_
- **When** _jag ska gå vidare till betalning_
- **Then** _vill jag lätt kunna se priserna_

#### User Story 3: Lägga produkter i varukorgen

- **Som en** _kund som väljer en produkt_
- **vill jag** _lägga till produkten i kundkorgen_
- **så att** _jag kan köpa den senare._

**Acceptanskriterier:**

- **Given** _att jag hittar en produkt jag vill ha_
- **When** _när jag klickar på lägg i varukorgknappen_
- **Then** _ska produkten läggas i varukorgen och interfacet uppdateras för att visa ändringarna._

#### User Story 4: Sida med detaljerad produktinformation

- **Som en** _användare intresserad av viss_
- **vill jag** _kunna kolla på detaljerade information om en enskild produkt_
- **så att** _jag kan ser bra bilder på den och sådant._

**Acceptanskriterier:**
**Given** att jag hittat en lovande produkt i katalogen
**When** jag clicka på den (kortet) 
**Then** kommer jag hamna på en sida med detaljerade produktinformation och snygga högupplösta bilder
...
2. **Modul 2:** `[t.ex. Autentisering med Clerk]`
   - **ADR-dokument:** Länk till `docs/ADR-002-[namn].md` _(frivillig andra ADR)_
   - **Kort motivering:** `[Varför valde ni denna lösning och vilka alternativ valdes bort?]`
...
- [ ] Pull Request är skapad och granskad (Code Review) av minst två andra teammedlemmar.
- [ ] Mergad till `dev`-branchen.
